...
% data processing (CalculateREWSfromLidarData_LDP_v3)!
% Results 4BeamPulsed:
% v3: Cost for Summer Games 2025 ("18 m/s hurdles"):  0.516059 m/s 
% Results CircularCW:
% v3: Cost for Summer Games 2025 ("18 m/s hurdles"):  0.463854 m/s 
...
switch LidarType
    case '4BeamPulsed'
        ...
        LDP.omega_cutoff        = 0.13;     % Corner frequency (-3dB) of the low-pass filter [rad/s]
        LDP.T_buffer            = 0.2;      % Buffer time for filtered REWS signal [s]
    case 'CircularCW'
        ...
        LDP.AngleToCenterline   = 15;       % Angle to centerline [deg]
        LDP.IndexGate           = 1;        % IndexGate
        LDP.FlagLPF             = 1;        % Enable low-pass filter (flag) [0/1]
        LDP.omega_cutoff        = 0.20;     % Corner frequency (-3dB) of the low-pass filter [rad/s]
        LDP.T_buffer            = 4.2;      % Buffer time for filtered REWS signal [s]      
end
...
for iSeed = 1:nSeed    
    ...
    Data                = table2struct(readtable(SolisResultFile),'ToScalar',true);
    ...
    REWS_WindField_Fs                   = interp1(Time_WindField,REWS_WindField,DataLDP.time);
    REWS_WindField_Fs_shifted           = interp1(Time_WindField-tau,REWS_WindField,DataLDP.time);     % shift the REWS from wind field by tau (intented prediction time) into the future (lower times)
    Error                               = REWS_WindField_Fs_shifted-DataLDP.REWS_b;                    % error is  REWS from wind field shifted minus REWS from lidar filtered and buffered.
    MAE(iSeed)                          = mean(abs(detrend(Error(DataLDP.time>=t_start),'constant'))); % only consider error after t_start 
             
    % Plot REWS for absolute error             
    figure('Name',['REWS seed ',num2str(Seed)])
    subplot(311)
    hold on; grid on; box on
    plot(DataLDP.time,  REWS_WindField_Fs);
    plot(DataLDP.time,  DataLDP.REWS);
    ylabel('REWS [m/s]');
    legend('wind field','lidar estimate')
    subplot(312)
    hold on; grid on; box on
    plot(DataLDP.time,  REWS_WindField_Fs_shifted);
    plot(DataLDP.time,  DataLDP.REWS_b);
    ylabel('REWS [m/s]');
    legend('wind field shifted','lidar estimate filtered and buffered')
    subplot(313)
    hold on; grid on; box on
    plot(DataLDP.time,  Error);
    ylabel('error [m/s]');
    xlabel('time [s]')
